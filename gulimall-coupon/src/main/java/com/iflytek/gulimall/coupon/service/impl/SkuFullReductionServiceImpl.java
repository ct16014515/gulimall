...
public class SkuFullReductionServiceImpl extends ServiceImpl<SkuFullReductionDao, SkuFullReductionEntity> implements SkuFullReductionService {
    ...
    @Override
    public void saveSkuReduction(SkuReductionVO skuReductionVO) {

        //1、保存满减打折、会员价
        //1、1）、sku的优惠、满减等信息：gulimall_sms--->sms_sku_ladder、sms_sku_full_reduction、sms_member_price
        SkuLadderEntity skuLadderEntity = new SkuLadderEntity();
        BeanUtils.copyProperties(skuReductionVO,skuLadderEntity);
        skuLadderEntity.setAddOther(skuReductionVO.getCountStatus());

        if (skuReductionVO.getFullCount() > 0) {
            skuLadderService.save(skuLadderEntity);
        }

        //2、sms_sku_full_reduction
        SkuFullReductionEntity skuFullReductionEntity = new SkuFullReductionEntity();
        BeanUtils.copyProperties(skuReductionVO,skuFullReductionEntity);
        ...
        List<MemberPriceEntity> collect = memberPrice.stream().map(mem -> {
            ...
            memberPriceEntity.setSkuId(skuReductionVO.getSkuId());
